...
public class BookTourController extends HttpServlet {
    ...
    private static final String INVALID = "view_cart.jsp";
        ...
        try {
            ...
            if (cart != null) {
                ...
                for (BookingTourDTO dto : cart.getCart().values()) {
                        ...
                        if (!CartValidation.checkQuantity(dto.getAdultQuantity() + dto.getKidQuantity(), dto.getCode())) {
                            valid = false;
                            err += dto.getCode() + " ";
                        }
                ...
                if (!err.equals("")) {
                    errorObj.setQuantityError(err + "is out of tickets.");
                }
                if (valid) {
                    CartBean beans = new CartBean();
                    beans.setCart(cart);
                    if (beans.insertBookingDetail()) {
                        url = SUCCESS;
                        cart.getCart().clear();
